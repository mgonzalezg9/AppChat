...
/**
 * El catálogo mantiene los objetos en memoria, en una tabla hash para mejorar
 * el rendimiento. Esto no se podría hacer en una base de datos con un número
 * grande de objetos. En ese caso se consultaria directamente la base de datos
 */
public class UsersCatalogue {
	...
	public Optional<User> getUsuarioNumTelf(int numTelefono) {
		return usuarios.values().stream().filter(u -> u.getNumTelefono() == numTelefono).findAny();
	}
